...
| Div -> (/)
| Mod -> (mod)
| b -> raise (Error_no_pos "warning:" ^ string_of_binop b "is not an arithmetic operation")
...
| Neq -> (!=)
| Eq -> (=)
| b -> raise (Error_no_pos  "warning:" ^ string_of_binop b "is not a comparison operation")

let convert_cond (o:binop) = match o with
| And -> (&&)
| Or -> (||)
| b -> raise (Error_no_pos  "warning:" ^ string_of_binop b "is not an conditional operation")
...
let rec eval_expr hashtable_loc e =
  ...
  | Val v -> (match v with
    | Var x -> (match Hashtbl.find_opt hashtable_loc x with
      | None -> raise (Error_no_pos "Undefined value" ^ x)
      | Some i -> i)
  )
  | Moins e ->
