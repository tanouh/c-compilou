...
exception Error_ret_void of i_expr
...
let rec eval_expr hashtable_loc e =
  ...
  | Val Var x -> (match Hashtbl.find_opt hashtable_loc x with
    ...
    | Ecall (name,expl) -> ( match Hashtbl.find_opt functions name with 
      ...
      | Some (Dvoid, l) -> raise (Error_ret_void (check_call hashtable_loc name expl)) 
      | Some (Dint, l) -> check_call hashtable_loc name expl )
    ...
    | e -> try ( match e with
      ...
      | Op (op,e1,e2) ->
        ...
        | Add | Sub | Mul | Div | Mod -> (match (eval_expr hashtable_loc e1, eval_expr hashtable_loc e2) with
          ...
          |(ie1,ie2) -> Ibinop (op,ie1,ie2)) (* on ne fait rien car ce ne sont pas directement deux constantes *)
      ...
      | Error_ret_void i_e-> raise (Error_no_pos ("error: void value not ignored as it ought to be"))
    ...
    and check_call hashtable_loc name expl =
      if Hashtbl.mem functions name then ( (* la fonction name est déjà définie *)
        let args_compile =  List.map (eval_expr hashtable_loc) expl in (* on compile d'abord les arguments que prend name*)
        let expr_to_type expr = match expr with (* la fonction expr_to_type permettra de comparer les types des éléments de expl avec les types des arguments attendus par name *)
...
let compile_program p ofile =
  load_print_int;
  let aux x =
    if Hashtbl.mem functions x.name then raise (Error_no_pos ("error: redefinition of " ^ x.name)) (* C ne permet pas de déclarer deux fonctions avec le même nom*)
    else  Hashtbl.add functions x.name (x.ret_type, List.map fst x.args); (* ajoute la nouvelle variable gloable/fonction à la table*)
    match x.body with
    | None -> (x.name, 0 ,List.length x.args, Iassign((Iglobal x.name, 4), IUndef)) (* c'est une variables globale *)
